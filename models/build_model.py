...
def _parse_upsample(x, layers, size, interpolation):
...
def mask_proto(x, layers, decay_factor, npr, nm):
    ...
    x,_=_parse_convolutional(x, layers, decay_factor,  npr, kernel_size=3)
    x,_=_parse_convolutional(x, layers, decay_factor,  nm)
    return x, layers
...
def _parse_segmment(x, layers, decay_factor, nc=80, anchors=(), nm=32, npr=256, ch=(),  imgsz=(640, 640), training=False):
...
def _parse_c3(x, layers, decay, n,kernel_size, stride, c1, filters, pad=1, bn=1, activation=1):
    e = 0.5
    c_ = int(filters * e)  # hidden channels
    x1,_=_parse_convolutional(x, layers, decay, c_, kernel_size, stride, pad, bn=1, activation=1)
    for idx in range(n):
...
def _parse_bottleneck(x, c1, c2, decay=0.01, shortcut=True, g=1, e=0.5):
    ...
    kernel_size=1
    stride=1
    x1 ,_= _parse_convolutional(x, layers, decay, c_, kernel_size, stride)
    kernel_size=3
    x1,_=_parse_convolutional(x1, layers, decay, c2, kernel_size, stride)
...
def _parse_convolutional(x, layers, decay, filters, kernel_size=1, stride=1, pad=1, bn=1, activation=1):
...
def parse_model(inputs, na, nc, gd, gw,mlist, ch, imgsz, decay_factor):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(mlist):  # mlist-list of layers configs. from, number, module, args
        ...
        if m_str == 'Conv':
            ...
            x, layers = _parse_upsample(x, layers, size, interpolation)
        elif m_str == 'Concat':
